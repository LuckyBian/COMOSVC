...
def load_teacher_model_with_pitch(model, checkpoint_dir):
    # 加载教师模型
    model_resumed = torch.load(checkpoint_dir, map_location="cpu")
    model.load_state_dict(model_resumed['model'], strict=False)

    # 安全加载 pe 模型（可选）
    pe_ckpt_path = './m4singer_pe/model_ckpt_steps_280000.ckpt'
    if os.path.exists(pe_ckpt_path):
        model_pe_resumed = torch.load(pe_ckpt_path, map_location="cpu")['state_dict']
        prefix_in_ckpt = 'model'
        model_pe_resumed = {
            k[len(prefix_in_ckpt) + 1:]: v
            for k, v in model_pe_resumed.items()
            if k.startswith(f'{prefix_in_ckpt}.')
        }

        # 判断 decoder 是否有 pe 模块
        if hasattr(model.decoder, "pe"):
            try:
                model.decoder.pe.load_state_dict(model_pe_resumed, strict=True)
                print("✅ Successfully loaded decoder.pe weights.")
            except Exception as e:
                print(f"⚠️ Failed to load decoder.pe: {e}")
        else:
            print("⚠️ Skipped loading decoder.pe (not defined in model).")
    else:
        print(f"⚠️ PE checkpoint not found at {pe_ckpt_path}, skipping.")

    # 深拷贝 denoise_fn 用于 EMA / 蒸馏对比
    model.decoder.denoise_fn_ema = copy.deepcopy(model.decoder.denoise_fn)
    model.decoder.denoise_fn_pretrained = copy.deepcopy(model.decoder.denoise_fn)
