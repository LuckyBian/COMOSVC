...
class Svc(object):
    def __init__(self,
        ...
        self.hubert_model = ContentVec256L9(device = self.dev)
    ...
    def get_unit_f0(self, wav, tran):
        ...
        # f0=f0-np.mean(f0)+203.7736179631715
    ...
    def infer(self, speaker, tran, raw_path):
        ...
        sid_mel='./dataset/'+speaker+'/'+speaker+'.spknew.npy'
        mel_ref = np.load(sid_mel,allow_pickle=True)
        mel_ref=torch.tensor(mel_ref)
        mel_ref=torch.squeeze(mel_ref)
        sid=mel_ref.cuda().unsqueeze(0)
    ...
    def slice_inference(self,
        ...
        for (slice_tag, data) in audio_data:
            ...
            for k,dat in enumerate(datas):
                ...
                print("Zeros shape (before):", np.zeros([pad_len]).shape)

                try:
                    print("Data type of dat:", dat.dtype)
                except Exception as e:
                    print("Error accessing data type of dat:", e)
                    # 如果可能，重置dat的dtype
                    dat = dat.astype(np.float32)

                zeros_array = np.zeros(pad_len, dtype=np.float32)

                print(zeros_array.shape)


                dat = np.concatenate([zeros_array, dat, zeros_array])
