...
class AudioDataset(Dataset):
    def __init__(
        ...
        spk_mel=defaultdict(list)
        train_file= []
        # with open('/data/yiwenl/CoMoSVC_all/CoMoSVCmulti/filelists/finaltrain.txt') as f:
        ...
        #     #print(singer)
        #     spk_mel[spk].append(filename)

        # self.spk_mel=spk_mel
        for name_ext in tqdm(self.paths, total=len(self.paths)):
            path_audio = name_ext
            duration = librosa.get_duration(filename = path_audio, sr = self.sample_rate)
            
            path_f0 = name_ext + ".f0.npy"
            f0,uv = np.load(path_f0,allow_pickle=True)
            f0 = torch.from_numpy(np.array(f0,dtype=float)).float().unsqueeze(-1).to(device)
            uv = torch.from_numpy(np.array(uv,dtype=float)).float().unsqueeze(-1).to(device)
            ...
            # idx=random.randint(0,len(self.spk_mel[spk_name])-1)
            # filename_ref = self.spk_mel[spk_name][idx]
            # mel_ref=filename_ref+'.spknew.npy'
            mel_ref='./dataset/'+spk_name+'/'+spk_name+'.spknew.npy'
            mel_ref = np.load(mel_ref, allow_pickle=True)
            mel_ref = torch.tensor(mel_ref)
            spk_id = torch.squeeze(mel_ref).to(device)
            ...
            if load_all_data:
                ...
                path_units = name_ext + ".new.soft.pt"
    ...
    def get_data(self, name_ext, data_buffer):
        ...
        # uv = uv[start_frame : start_frame + units_frame_len]
        ...
        return dict(mel=mel, f0=f0_frames,volume=volume_frames, units=units, spk_id=spk_id, name=name, name_ext=name_ext)
